...
/** "3 entries · avg 6.2/10" — the day's log at a glance, one line. */
function summarizeDay(entries: SymptomEntry[]): string {
  const count = entries.length;
  const avg = entries.reduce((sum, e) => sum + e.severity, 0) / count;
  const entryWord = count === 1 ? 'entry' : 'entries';
  return `${count} ${entryWord} · avg ${avg.toFixed(1)}/10`;
}
              ...
              <Card style={styles.dayCard}>
                {group.entries.map((entry) => (
                  <EntryListItem
                    ...
                    entry={entry}
                    onPress={() =>
                      router.push({ pathname: '/log', params: { entryId: entry.id } })
                    }
                  />
                ))}
              </Card>
